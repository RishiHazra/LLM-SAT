...
class DataMakerCNFk(DataMaker):
    ...
    def generate(
        ...
        N: int,
        dimacs: bool,
    ) -> List[Tuple[int, str, int, int, List[List[int]]]]:
        log_path: str = os.path.join(data_dir, "log.txt")
        logfile = open(log_path, "w")

        tot_formulas: int = (
            np.floor(((alpha_max - alpha_min) / alpha_inc) + 1) * N * len(ns)
        )
        ...
        for n in ns:
            ...
            while alpha <= alpha_max:
                ...
                for N_idx in range(N):
                    ...
                    m = int(
                        round(alpha * n, 0)
                    )  # int() alone could introduce errors, i.e. 122.9999999 becomes 122
                    assert m / n == alpha, (
                        "Size '%s' for formulas leads to wrong alpha (%s instead of %s)"
                        % (n, m / n, alpha)
                    )
                    for _ in range(m):
                    ...
                    if dimacs:
                        ...
                        with open(
                            os.path.join(data_dir, dimacs_path), "w"
                        ) as dimacs_file:
                            dimacs_file.write(self.to_dimacs(n, formula))
                        log_details.append(dimacs_path)

                    formulas.append(
                        (formulas_idx, n, m, formula, is_sat(n, formula), dimacs_path)
                    )

                    log_str: str = "%d/%d formula generated (%s)" % (
                        formulas_idx,
                        tot_formulas,
                        ", ".join(log_details),
                    )
    ...
    # in the same directory of the original dataset as 'dataset_3sat_float.pkl'
    @staticmethod
    def remove_float_alpha(dataset_path: str):
        ...
        with open(dataset_path, "rb") as f:
            ...
            with open(os.path.join(dir, "dataset_3sat_float.pkl"), "wb") as f:
...
class DataMakerSR(DataMaker):
    ...
    def generate(
        ...
        dimacs: bool,
        iid: bool,
    ) -> List[Tuple[int, str, int, int, List[List[int]], bool]]:
        log_path: str = os.path.join(data_dir, "log.txt")
        logfile = open(log_path, "w")
        ...
        while pair_idx <= n_pairs:
            ...
            formula_pair: List[Tuple[bool, List[int]]] = [
                (False, clause_unsat),
                (True, clause_sat),
            ]
            ...
            for is_sat, last_clause in formula_pair:
                ...
                if dimacs:
                    dimacs_path = (
                        f"dimacs/sr_n={n_vars}_p={pair_idx}_sat={int(is_sat)}.cnf"
                    )
                ...
                formulas.append(
                    (formulas_idx, dimacs_path, n_vars, n_clauses, formula, is_sat)
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Dump DIMACS files (def. False)",
    )
    ...
    cnf_parser.add_argument(
        "-k", type=int, default=3, help="Size of the clauses (def. 3)"
    )
    cnf_parser.add_argument(
        "--ns",
    ...
    cnf_parser.add_argument(
        "--alpha_inc", type=float, default=0.2, help="Minimum alpha (def. 0.2)"
    )
    cnf_parser.add_argument(
        "-N",
        type=int,
        default=200,
        help="Number of formulas per alpha value (def. 200)",
    )
    ...
    if args.task == "sr":
        ...
        formulas = sr.generate(
            args.data_dir, args.n_pairs, args.min_n, args.max_n, args.dimacs, args.iid
        )
        ...
        DataMakerCNFk.remove_float_alpha(
            os.path.join(args.data_dir, "dataset_3sat_int.pkl")
        )
    else:
        raise NotImplementedError("Unsupported task")

    pickle_path = os.path.join(args.data_dir, "dataset_3sat_int.pkl")
